...
function DockItem({
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className={`cursor-pointer relative inline-flex items-center justify-center rounded-xl bg-zinc-900/70 border-gray-400 border-2 shadow-md ${className}`}
      tabIndex={0}
      role="button"
      aria-haspopup="true"
    >
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className={`${className} absolute -top-6 left-1/2 w-fit whitespace-pre rounded-md border border-neutral-700 bg-zinc-700 px-2 py-0.5 text-xs text-white`}
          role="tooltip"
          style={{ x: '-50%' }}
        >
          {children}
